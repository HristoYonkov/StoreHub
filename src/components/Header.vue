...
<template>
  <header class="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16 md:h-20">
        ...
        <nav class="hidden lg:flex items-center gap-1">
          <button v-for="cat in categories" :key="cat" @click="navigate(cat)" :class="[
        ...
        <div class="flex items-center gap-1">
          ...
          <!-- Cart -->
          <RouterLink to="/cart" class="relative p-2 rounded-full transition-colors"
    ...
    <!-- Mobile menu -->
    <div
      :class="['overflow-hidden transition-all duration-300 lg:hidden border-t border-gray-100', mobileOpen ? 'max-h-80' : 'max-h-0']">
      <nav class="flex flex-col px-4 py-3 gap-1 bg-white">
        ...
        <RouterLink to="/cart" @click="mobileOpen = false" :class="[
