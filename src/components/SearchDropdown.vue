...
<template>
  <div class="relative">
    <button @click="isOpen = !isOpen; $nextTick(() => inputRef?.focus())"
    ...
    <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2"
      ...
      <div v-if="isOpen"
        ...
        <div class="flex items-center gap-2 px-4 py-3 border-b border-gray-100">
          ...
          <input ref="inputRef" v-model="query" @focus="open" @blur="close" type="text" placeholder="Search products..."
        ...
        <ul v-if="results.length > 0" class="max-h-80 overflow-y-auto divide-y divide-gray-50">
          <li v-for="product in results" :key="product.id" @click="goToProduct(product.id)"
